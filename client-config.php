...
<body id="bclientconfig">
		...
		if ((isset($_GET['action'])) and ($_GET['action'] == "restart")){
			if (! (isset($_SESSION['password']))){
				...
				echo "<pre>Creating backup of current (working) config! sudo cp $client_config_file_default $client_config_file_default.bak";
				echo $ssh->exec("sudo cp $client_config_file_default $client_config_file_default.bak");
				echo "</pre>";
				echo str_repeat(' ',1024*64);
				echo "<pre>executing sudo mv $cwdir/$config_file_temp $client_config_file_default<br />";
				echo $ssh->exec("sudo mv $cwdir/$config_file_temp $client_config_file_default");
				echo "</pre>";
				echo str_repeat(' ',1024*64);
		...
		if ((isset($_GET['action'])) and ($_GET['action'] == "revert")){
				...
				echo "</pre>";
				echo str_repeat(' ',1024*64);
				echo "<pre>Restoring backup(working) config! sudo mv $client_config_file_default.bak $client_config_file_default";
				echo $ssh->exec("sudo mv $client_config_file_default.bak $client_config_file_default");
				echo "</pre>";
				echo str_repeat(' ',1024*64);
				$start_openvpn = $ssh->exec("/etc/init.d/openvpn start");
				if (stristr($start_openvpn, 'failed')){
